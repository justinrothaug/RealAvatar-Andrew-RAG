...
def get_chatassistant_chain():
    ...
    embeddings_model = OpenAIEmbeddings(openai.api_key= st.secrets["api_key"])
    vectorstore = FAISS.from_documents(documents, embeddings_model)
    llm = ChatOpenAI(model="ft:gpt-3.5-turbo-0125:personal::93Td8brn", temperature=1)
    memory = ConversationBufferMemory(memory_key='chat_history', return_messages=True) 
    chain = ConversationalRetrievalChain.from_llm(llm=ChatOpenAI(openai.api_key= st.secrets["api_key"]),
